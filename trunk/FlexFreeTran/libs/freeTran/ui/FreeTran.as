...
package ui{
	...
	public class FreeTran extends Sprite{
		...
		[Inspectable(defaultValue=true,name="边缩放")]
		public function set canLineScale(_canScale:Boolean):void{
			dot101.visible=dot110.visible=dot120.visible=dot102.visible=_canScale;
		}
		...
		private var targetstage:*;

		public function FreeTran(s:* = 0){
			if(s==0)
			{
				s=stage;
			}
		...
		private function added(event:Event):void{
			...
			targetstage.addEventListener(MouseEvent.MOUSE_UP,mouseUp);
			targetstage.addEventListener(MouseEvent.MOUSE_MOVE,mouseMove);
		...
		private function removed(event:Event):void{
			...
			targetstage.removeEventListener(MouseEvent.MOUSE_UP,mouseUp);
			targetstage.removeEventListener(MouseEvent.MOUSE_MOVE,mouseMove);
		...
		private function updateDotsByDragArea():void{
			...
			for each(var dot1:FreeTranDot1 in dot1Arr){
				...
				*/
				...
				//改成不是直接调节点的位置
				var dx:Number=dot1.x-p.x;
				var dy:Number=dot1.y-p.y;
				if(dx*dx+dy*dy>1){
					dot1.x=dot2.x=p.x;
					dot1.y=dot2.y=p.y;
				}
